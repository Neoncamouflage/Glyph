...
class Register(commands.GroupCog, name="register"):
    ...
    async def character(self, interaction: discord.Interaction, campaign_name: str, character_name: str) -> None:
        ...
        async with self.bot.db.acquire() as conn:
            async with conn.cursor() as cursor:
                try:
                    # Get the campaign ID based on the selected campaign name
                    ...
                    campaign_id = campaign_id[0]  # Fetch the ID from the tuple
    ...
    # Auto-complete method for the campaign selection
        ...
        async with self.bot.db.acquire() as conn:
            async with conn.cursor() as cursor:
                try:
                    ...
                    # Filter and return the list of campaigns that match the user's current input
                    return [
